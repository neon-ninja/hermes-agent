...
| `github_author_association` | No | Optional GitHub-only middleware for `issues` / `issue_comment` events. Gates LLM access by `author_association`. Supports `enabled` (default `true`), `allowed` (default `["OWNER","MEMBER","COLLABORATOR"]`), and `denied_message` (optional template used when access is denied). |
